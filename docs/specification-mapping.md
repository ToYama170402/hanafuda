...
**該当Issue:** MVP機能群（プロジェクト構造、Core層、札定義、状態管理、山札管理、基本役判定、得点計算、ターン進行、アクション、ファサード、配札検証、MVP統合テスト、こいこい、フェーズ2統合）
...
**該当Issue:** 拡張機能群（特殊役、総流れ、配札再検証、ルール設定、イベントシステム、エラーハンドリング、APIリファレンス、CI/CD）
...
**該当Issue:** オプション機能群（手四、フェーズ3統合、最適化、ログ、リプレイ、フェーズ4統合）
...
フェーズ2統合 → 手四 → フェーズ3統合 → 最適化 → フェーズ4統合
            → ルール設定 ↗              → ログ ↗
                                        → リプレイ ↗
...
```
プロジェクト構造 → Core層データモデル → 札定義 → 山札管理 → 基本役判定 → 得点計算 → 状態管理 → ターン進行 → アクション → ファサード → 配札検証 → MVP統合テスト
```
