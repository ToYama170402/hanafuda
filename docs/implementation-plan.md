...
### フェーズ1内の並行可能タスク
- Issue #2（データモデル）とIssue #4（状態管理）は並行可能
- Issue #6（役判定）とIssue #8（ターン進行）は部分的に並行可能

### フェーズ2内の並行可能タスク
- Issue #14（喰い付き）、#15（特殊役：種）、#16（特殊役：短冊）は並行可能
- Issue #17（総流れ）とIssue #18（配札再検証）は並行可能

### フェーズ3内の並行可能タスク
- Issue #20（手四）、#21（ルール設定）、#22（イベント）は並行可能

### その他
- Issue #28（エラーハンドリング）は各フェーズと並行実装可能
- Issue #30（CI/CD）はプロジェクト初期から並行実装可能
...
### 即座に実行すべきこと
1. **Issue #1**: プロジェクト構造のセットアップ（最優先）
2. **Issue #30**: CI/CDパイプラインの基本設定（並行実行）
3. **Issue #2**: Core層のデータモデル実装（次の優先）
