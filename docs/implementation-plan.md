...
- [ ] 配札ロジックの実装（子4枚→親4枚→場4枚→子4枚→親4枚→場4枚）
- [ ] 山札からのドロー機能
- [ ] シャッフルのテスト
...
**優先度**: 高  
**依存関係**: フェーズ2の全タスク完了（こいこい、喰い付き、特殊役、総流れ、配札再検証）  
**推定工数**: 6-8時間
...
**優先度**: 中  
**依存関係**: フェーズ3の全タスク完了（手四、ルール設定、イベントシステム）  
**推定工数**: 4-5時間
...
### フェーズ1内の並行可能タスク
- 「Core層の基本データモデル実装」と「ゲーム状態管理クラスの実装」は並行可能
- 「基本的な役判定の実装（光札、種、短冊、カス）」と「ターン進行の基本ロジック」は部分的に並行可能

### フェーズ2内の並行可能タスク
- 「喰い付き（くいつき）の実装」、「特殊な役の実装（猪鹿蝶、花見で一杯、月見で一杯）」、「短冊の特殊役（赤短、青短）の実装」は並行可能
- 「総流れ（そうながれ）の実装」と「配札の再検証機能」は並行可能

### フェーズ3内の並行可能タスク
- 「手四（てし）の実装（オプション）」、「ルール設定のカスタマイズ機能」、「イベントシステムの実装」は並行可能

### その他
- 「エラーハンドリングの実装」は各フェーズと並行実装可能
- 「CI/CDの設定」はプロジェクト初期から並行実装可能
...
### 即座に実行すべきこと
1. **「プロジェクト構造とソリューションのセットアップ」**: プロジェクト構造のセットアップ（最優先）
2. **「CI/CDの設定」**: CI/CDパイプラインの基本設定（並行実行）
3. **「Core層の基本データモデル実装」**: Core層のデータモデル実装（次の優先）
